...
def generate_data(n):
    ...
    if not DIRECT:
        Y_fair = (gender_effect_on_income * S +
            sector_effect_on_income * M  - (prob_sector_1-prob_sector_0)*M*S*sector_effect_on_income+
            beta_W * W + noise
            )
    else:
    ...
    if BINARY:
        ...
        data.to_csv(f'Generated{n}_binary_fair.csv', index=False, encoding='utf-8')

    else:
        data = pd.DataFrame({'Gender': S, 'Sector': M, 'Worked_Hours': W, 'Income': Y, 'Income_Fair': Y_fair})
        data.to_csv(f'Generated{n}_fair.csv', index=False, encoding='utf-8')
